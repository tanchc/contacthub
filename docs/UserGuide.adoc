...
=== Adding a photo to a Person: `addPhoto` or `ap` [Since ?]
...
* These commands open a new window to view the described information.
...
Opens the official reference for NUS bus routes. +
Format: `bus`

=== Viewing NUS map : `map` or `m` [Since v1.5rc]

Opens the official map for NUS. +
Format: `map`
