...
The Find Tag mechanism is implemented using the `FindTagCommand`, which resides inside `Logic`. It supports listing all the contacts in the address book associated with.

When the user requests for contacts associated to a tag name(s), the list of contacts will be created and shown.

`FindTagCommand` inherits from Command and is implemented this way.
[source,java]
----
...
The user executes `FindTagCommand`, the list will now show the contacts associated to the tag name(s) keyed in.

image::FindTagCommand.png[width="300"]

The user can continue to add more tag names for the FindTagCommand, and if any contacts contains any of the tag name(s) keyed in, they will show up

[NOTE]
Upon exiting ContactHub, the contact list will not be filtered for according to the last tag name(s) keyed in in the next launch.

==== Design Considerations

**Aspect:** Implementation of `FindTagCommand` +
**Alternative 1 (current choice):** `FindTagCommand` inherits from `Command`. +
**Pros:** After filtering, the contact list remains will only show contacts associated to the tag name(s) keyed in. +
...
**Cons:** `FindTagCommand` must be extended from `UndoableCommand` and must remember to call `super.execute()`, or lose the ability to undo/redo.
...
**Alternative 2:** Creates a new findTag algorithm. +
...
The `commandBox` class also instantiates an object of `TextField` class which is a JavaFX FXML, an XML-based language for building a user interface.
The `TextField` class inherits the `TextInputControl` class which is the base class for text inputs.

Suppose the user wants to delete everything in the command box. The user then executes the escape key on the user's keyboard. The `commandBox` class
 would then call the method setText() in the `TextInputControl` class, which is a superclass of `TextField` class, which
then sets the text in the command box to be empty. Calling it multiple times would involve the same result.

In future, there would be more keyboard shortcuts like displaying all the prefixes of the AddCommand in the command box
 or keyboard shortcuts that would make a help guide appear.
...
=== Deselect Function (To be added)

The implementation of this function is largely
to facilitate the usage of the default panel (default.html called by BrowserPanel.java) as a quick reference
for ContactHub commands. This was inspired by the IntelliJ default text when no files are selected in a project.

The deselect function will be implemented through the clearSelection() method in the SelectionModel class. The initial planned implementation of this will be through the CLI but
GUI support is also planned. We want the deselection to be intuitive and work exactly like the pre-existing selection function.

Initially, a splash screen was considered to show the commands. However, this would not be ideal as users would not be able to return back to it. In addition, it would slow down the startup time for experienced users. For the purpose of deselection itself, there is not really any alternative and in the absence of a command reference default panel, there is little purpose for it.
...
|`* * *` |user |add multiple tags to a person |my contacts are more recognizable

|`* * *` |user |have different coloured tags |differentiate my contacts easier
...
|`* *` |user |import contacts from iCloud |
...
|`* *` |user |search by tags |filter by tags only
...
2. ContactHub shows a list of tags in the address book.
+
Use case ends.
...
1.  User requests to find a person(s) with the searched tag name
2.  ContactHub displays a list of person(s) with the tag name keyed in
+
Use case ends.
...
[none]
* 2a. ContactHub does not contain anyone with the tag.
+
Use case ends.
