...
The Find Mod mechanism is implemented using the `FindModCommand`, which resides inside `Logic`. It supports listing all the contacts in the address book associated with.

When the user requests for contacts associated to a mod name(s), the list of contacts will be created and shown.

`FindModCommand` inherits from Command and is implemented this way.
[source,java]
----
...
The user executes `FindModCommand`, the list will now show the contacts associated to the mod name(s) keyed in.

image::FindModCommand.png[width="300"]

The user can continue to add more mod names for the FindModCommand, and if any contacts contains any of the mod name(s) keyed in, they will show up

[NOTE]
Upon exiting ContactHub, the contact list will not be filtered for according to the last mod name(s) keyed in in the next launch.

==== Design Considerations

**Aspect:** Implementation of `FindModCommand` +
**Alternative 1 (current choice):** `FindModCommand` inherits from `Command`. +
**Pros:** After filtering, the contact list remains will only show contacts associated to the mod name(s) keyed in. +
...
**Cons:** `FindModCommand` must be extended from `UndoableCommand` and must remember to call `super.execute()`, or lose the ability to undo/redo.
...
**Alternative 2:** Creates a new findMod algorithm. +
...
1. User requests to list all mods in the address book.
2. ContactHub shows a list of mods in the address book.
+
Use case ends.
...
1.  User requests to find a person(s) with the searched mod name
...
[none]
* 2a. ContactHub does not contain anyone with the mod.
+
Use case ends.
