...
The FindModule mechanism is implemented using the `FindModuleCommand`, which resides inside `Logic`. It supports listing all the contacts in the address book associated with.
...
The user can continue to add more mod names for the FindModuleCommand, and if any contacts contains any of the module(s) keyed in, they will show up
...
1. User requests to list all module(s) in ContactHub.
2. ContactHub shows a list of module(s) in ContactHub.
+
Use case ends.
