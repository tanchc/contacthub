...
The following sequence diagram shows how the list module operation works:

image::ListModulesSequenceDiagram.png[width="800"]
...
The following sequence diagram shows how the add photo operation works:

image::AddPhotoSequenceDiagram.png[width="800"]
...
image::keyboardDiagram.PNG[width="350"]
...
in the `ui` package. It provides the user the ability to check information on a given module or venue using only the CLI.
This is broken up into two different commands, the `getmodule` command and the `venue` command.
Both commands are relatively similar in implementation, with the major difference being the
page loaded in the browser panel.

The `MainWindow` inherits the abstract class `UIPart` which represents a distinct part of the UI. e.g. Windows, dialogs, panels, status bars, etc.

image::NUSModsUIDiagram.png[width="900"]


The image shows the `getmodule` implementation, which is similar to the `venue` implementation.

Both commands are handled by their by their respective event handlers (in `MainWindow`) when their commands are entered,
which then call the `BrowserPanel.loadPage(arg)` method. `arg` being the module/venue
inputted by the user, which determines the module/venue information pulled.

For future versions, a URL redirection check is planned as currently entering a wrong module/venue name
simply redirects to the NUSMods homepage. Ideally, this would return an "invalid command format" to the user
and not load any webpage instead.
...
image::GoogleMapsUIDiagram.PNG[width="350"]
...
The bus and map display features are implemented through the use of two separate commands - `bus` and `map` commands.
These commands open NUS bus routes and the NUS map in a new window.

image::BusUIDiagram.png[width="900"]

The image shows the implementation of the `bus` command.

They are facilitated through the `MainWindow` class through their respective event handlers
that call the `show()` method for the given class (`BusWindow` or `MapWindow`). The respective FXML is then loaded
(`BusWindow.fxml` or `MapWindow.fxml`). The ScrollPane is particularly important for the map display,
as the NUS map image is relatively large.
...
The addition of the TaskList introduces new components into the Model. A `UniqueTaskList` class is created to hold many
`Task` objects. And each `Task` object will contain an instance of the `Appointment`, `Date`, and `StartTime` class.

image::TaskListPanelUIDiagram.PNG[width="350"]
...
|`* * *` |user |add multiple mods to a person |know which contacts are taking which modules

|`* * *` |user |have different coloured modules |make them easier to differentiate
...
1.  User requests for bus routes.
2.  ContactHub shows the bus routes to the user.
+
Use case ends.
...
1.  User requests for NUS Map.
2.  ContactHub shows the NUS Map to the user.
+
Use case ends.
...
1.  User requests for NUSMods module information for a given module.
2.  ContactHub shows the NUSmods module page for the given module to the user in the browser.
+
Use case ends.
...
1.  User requests for NUSMods venue information for a given venue.
2.  ContactHub shows the NUSmods venue page for the given venue to the user in the browser.
+
Use case ends.
...
.  ContactHub should work on any link:#mainstream-os[mainstream OS] as long as it has Java `1.8.0_60` or higher installed.
.  ContactHub should be able to hold up to 1000 persons without a noticeable sluggishness in performance for typical usage.
.  ContactHub should allow user with above average typing speed for regular English text (i.e. not code, not system admin commands) to accomplish most of the tasks faster using commands than using the mouse.
.  ContactHub source code should be open source, and conforms to coding standards.
.  ContactHub mean time to recovery must be 10 seconds maximum.
.  ContactHub must be user friendly even for non IT-savvy, conforms to user standards.
.  User has access to alphanumeric inputs to use command line interface.
.  User has access to Internet to access features.
...
.  User should get system response after entering a command within 5 seconds.
